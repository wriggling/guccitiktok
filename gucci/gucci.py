...
away = Colors.red_to_purple
welcome = Colors.green_to_blue
...
def main():
    ...
    session_id = Write.Input(""" 
+-------------------------------------+
|┏┓┳┳┏┓┏┓┳  ┏┳┓•┓ ┏┳┓  ┓              |
|┃┓┃┃┃ ┃ ┃   ┃ ┓┃┏ ┃ ┏┓┃┏             |
|┗┛┗┛┗┛┗┛┻   ┻ ┗┛┗ ┻ ┗┛┛┗             |
|This only works with arab countries. |
|Enter sessionid below.               |
+-------------------------------------+
↓↓↓
""", welcome, interval=0.01)

    user = get_profile(session_id, device_id, iid)
    if user != "None":
        print(Colorate.Vertical(welcome, f"Tiktok User has been changed to: {user}"))
        new_username = input(Colorate.Vertical(welcome, "Enter the username you want: "))
        Write.Print(change_username(session_id, device_id, iid, user, new_username))

    else:
        print(Colorate.Vertical(welcome, "Invalid session ID or other error."))
    print(Colorate.Vertical(welcome, "@cxcvc on Discord"))
