...
def main():
    ...
    try:
        ...
        s = """
                                         ...
                                         |┃┓┃┃┃ ┃ ┃   ┃ ┓┃┏ ┃ ┏┓┃┏             |
                                         |┗┛┗┛┗┛┗┛┻   ┻ ┗┛┗ ┻ ┗┛┛┗             |
                                         |This only works with Japan.          |
                                         |Enter sessionid below.               |
                                         +-------------------------------------+
                                         ↓↓↓
                                         """             
        s.center(40)
        session_id = Write.Input(f"""\n\n\n\n\n\n{s}""",  gucci, interval=0.003)

        user = get_profile(session_id, device_id, iid)
        clear()
        if user != "None":
            print(f"""\n\n\n\n\n\n                                         {Fore.GREEN}Tiktok User has {Fore.RED}been changed to: {user}
                  """)
            new_username = input(f"                                         {Fore.GREEN}Enter the username {Fore.GREEN}you want: ")
